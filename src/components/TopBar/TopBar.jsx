...
const TopBar = () => {
  ...
  return (
    <div className="text-white flex gap-2 py-2 px-1 text-sm overflow-x-auto">
      {isFetching ||
        data?.genres.map(({ name, id }) => (
          <Link
            key={id}
            value={id}
            className="text-white hover:bg-green-600 rounded-lg justify-center items-center flex px-2 py-2"
          >
